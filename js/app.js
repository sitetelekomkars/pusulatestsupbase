...
.arm { width: 16px; height: 45px; background: linear-gradient(135deg, #ffeb3b 0%, #fbc02d); border-radius: 10px; position: absolute; top: 0; box-shadow: inset 0 0 6px rgba(0,0,0,0.3); transform-origin: top center; }
...
.quality-controls-top { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; margin-bottom: 15px; flex-wrap: nowrap; gap: 15px; }
...
.criteria-controls {
    display: block; /* Eski flex yerine blok yaptık */
    background: #f9f9f9;
    padding: 10px;
    border-radius: 6px;
}
...
.eval-button-group { display: flex; gap: 8px; justify-content: center; align-items: center; }
.eval-button {
    padding: 8px 12px; border: 1px solid #ccc; border-radius: 4px; cursor: pointer;
    font-weight: 600; font-size: 0.8rem; transition: all 0.1s; flex-grow: 1;
    text-align: center; color: #555; background: #fff;
    white-space: nowrap; /* Metnin sığmasını sağla */
}
.eval-button:hover:not(.active) { border-color: var(--primary); background: #f0f4ff; }
/* İyi Puan: Yeşil */
.eval-good { border-color: #2e7d32; color: #1b5e20; background: #e8f5e9; }
.eval-good.active {
    background: #2e7d32; color: white; border-color: #1b5e20;
    box-shadow: 0 2px 4px rgba(46, 125, 50, 0.3);
}
/* Orta Puan: Turuncu */
.eval-medium { border-color: #ed6c02; color: #e65100; background: #fff3e0; }
.eval-medium.active {
    background: #ed6c02; color: white; border-color: #e65100;
    box-shadow: 0 2px 4px rgba(237, 108, 2, 0.3);
}
/* Kötü Puan: Kırmızı */
.eval-bad { border-color: #d32f2f; color: #b71c1c; background: #ffebee; }
.eval-bad.active {
    background: #d32f2f; color: white; border-color: #b71c1c;
...
.quality-fullscreen-wrapper {
    ...
    background: #f4f6f9; z-index: 9000; display: flex;
    font-family: 'Segoe UI', sans-serif;
}
/* SIDEBAR */
.q-sidebar {
    width: 260px; background: #0e1b42; color: #fff;
    display: flex; flex-direction: column;
    box-shadow: 4px 0 15px rgba(0,0,0,0.1); flex-shrink: 0;
}
.q-sidebar-header {
    height: 60px; display: flex; align-items: center; gap: 10px;
    padding: 0 20px; font-size: 1.2rem; font-weight: bold;
    color: #fabb00; border-bottom: 1px solid rgba(255,255,255,0.1);
}
.q-user-profile {
...
.q-avatar-circle {
    width: 40px; height: 40px; background: #fabb00; color: #0e1b42;
    border-radius: 50%; display: flex; align-items: center; justify-content: center;
    font-weight: bold; font-size: 1.2rem;
}
.q-user-name { font-weight: 600; font-size: 0.95rem; }
.q-user-role { font-size: 0.75rem; color: rgba(255,255,255,0.6); }
.q-nav-menu { list-style: none; padding: 20px 0; margin: 0; flex-grow: 1; }
.q-nav-item {
    padding: 12px 20px; cursor: pointer; color: rgba(255,255,255,0.7);
    transition: 0.2s; display: flex; align-items: center; gap: 12px; font-size: 0.95rem;
}
.q-nav-item:hover { color: #fff; background: rgba(255,255,255,0.05); }
.q-nav-item.active { background: #fabb00; color: #0e1b42; font-weight: bold; border-right: 4px solid #fff; }
.q-sidebar-footer { padding: 20px; border-top: 1px solid rgba(255,255,255,0.1); }
.q-exit-btn {
    width: 100%; padding: 10px; background: #d32f2f; border: none;
    color: white; border-radius: 6px; cursor: pointer; font-weight: bold;
    display: flex; align-items: center; justify-content: center; gap: 8px;
}
.q-exit-btn:hover { background: #b71c1c; }
/* CONTENT AREA */
.q-content { flex-grow: 1; padding: 30px; overflow-y: auto; background: #f4f6f9; position: relative; }
.q-view-section { display: none; animation: fadeIn 0.3s ease; }
.q-view-section.active { display: block; }
.q-view-header {
    display: flex; justify-content: space-between; align-items: center;
    margin-bottom: 25px; border-bottom: 2px solid #e0e0e0; padding-bottom: 15px;
}
.q-view-header h2 { margin: 0; color: #0e1b42; }
/* DASHBOARD WIDGETS */
.q-stats-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px; }
.q-stat-box {
    background: white; padding: 20px; border-radius: 12px;
    box-shadow: 0 4px 10px rgba(0,0,0,0.05); display: flex; align-items: center; gap: 20px;
    border-left: 5px solid transparent;
}
.box-blue { border-color: #1976d2; } .box-green { border-color: #2e7d32; } .box-orange { border-color: #ed6c02; }
.stat-icon {
    width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center;
    font-size: 1.5rem; color: white;
}
.box-blue .stat-icon { background: #1976d2; }
.box-green .stat-icon { background: #2e7d32; }
.box-orange .stat-icon { background: #ed6c02; }
.stat-info { display: flex; flex-direction: column; }
.stat-val { font-size: 1.8rem; font-weight: 800; color: #333; line-height: 1; }
.stat-label { font-size: 0.85rem; color: #777; margin-top: 5px; }
.q-dashboard-grid { display: flex; gap: 20px; }
.q-chart-card { background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); flex: 1; display:flex; flex-direction:column; align-items:center; }
.large-ring { width: 150px; height: 150px; margin: 20px auto; }
.large-ring .score-circle-inner { width: 120px; height: 120px; font-size: 2.5rem; }
.simple-list { width: 100%; text-align: left; }
.simple-item { padding: 10px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; }
.simple-item:last-child { border: none; }
.q-period-selector select { padding: 5px 10px; border-radius: 4px; border: 1px solid #ccc; font-size: 0.9rem; }
/* --- EĞİTİM KARTLARI --- */
.q-training-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; }
.t-card { 
    background: #ffffff; 
    border-radius: 8px; 
    overflow: hidden; 
    ...
    border-top: 4px solid var(--secondary); /* GÜNCEL STİL: Üstte sarı çizgi */
    transition: all 0.3s ease; 
}
.t-card:hover {
...
.t-card-body { padding: 15px; flex-grow: 1; color: #555; font-size: 0.95rem; line-height: 1.5; }
.t-card-footer { padding: 15px; background: #f8f9fa; border-top: 1px solid #eee; text-align: right; }
.t-status-badge { font-size: 0.7rem; padding: 3px 8px; border-radius: 12px; background: rgba(255,255,255,0.2); }
.t-btn { padding: 8px 16px; border-radius: 4px; border: none; cursor: pointer; font-weight: bold; transition: 0.2s; }
.t-btn-start { background: #fabb00; color: #0e1b42; }
.t-btn-start:hover { background: #ffc107; }
.t-btn-done { background: #2e7d32; color: white; cursor: default; }
.t-doc-link {
    display: inline-flex;
    align-items: center;
    gap: 8px;
...
.feedback-card {
    ...
    border-radius: 8px;
    margin-bottom: 15px;
    border-left: 4px solid #1976d2; /* Renk, JS tarafından overwrite edilir */
...
#manual-q-agent, #manual-q-topic, #manual-q-callid, #manual-q-date, #manual-q-type {
...
/* --- TEKNİK SİHİRBAZ STİLLERİ (YÜKSEK ÖNCELİKLİ) --- */
.tech-step-title {
    font-size: 1.4rem;
    margin-bottom: 25px;
    font-weight: 700;
...
/* Penalty Game - Double indicator */
.double-indicator{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:8px 12px;border-radius:999px;background:rgba(255,193,7,0.18);border:1px solid rgba(255,193,7,0.45);color:#ffc107;font-weight:800;letter-spacing:0.5px;margin:10px auto 0;max-width:max-content;}


/* --- Dashboard / Puan Durumu: Admin için temsilci ortalamaları --- */
.q-dash-agent-scores{
  margin-top: 14px;
  display: grid;
  gap: 8px;
...
.q-dash-agent-scores .das-left{
  display:flex;
  align-items:center;
  gap:8px;
  min-width: 0;
}
.q-dash-agent-scores .das-name{
  font-weight:700;
  color:#223;
  white-space:nowrap;
  overflow:hidden;
...
/* --- UX İyileştirmeleri: Sonuç Sayacı + Öne Çıkanlar --- */
.search-result-count{
  font-size:0.72rem;
  margin-top:6px;
...
.gh-icon{ font-size: 2rem; }
.gh-title{ margin-top:8px; font-weight:900; color: var(--primary); }
.gh-desc{ margin-top:6px; color:#667; font-size:0.9rem; line-height:1.45; }
.gh-cta{
  margin-top: 12px;
  display:inline-flex;
  align-items:center;
...
.quick-chip{
  padding: 8px 12px;
  border-radius: 999px;
  background: rgba(14,27,66,0.06);
  border:1px solid rgba(14,27,66,0.10);
  color:#223;
  font-weight:800;
  font-size:0.85rem;
}
...
.quick-opt.good{ border-color: rgba(46,125,50,0.35); background: rgba(46,125,50,0.08); }
.quick-opt.bad{ border-color: rgba(211,47,47,0.35); background: rgba(211,47,47,0.08); }
.quick-footer{
  margin-top: 14px;
  display:flex;
  justify-content:space-between;
...
/* --- HOME SCREEN --- */
.home-screen{animation:fadeIn .25s ease;padding-top:10px}
.home-hero{display:flex;gap:14px;align-items:stretch;justify-content:space-between;flex-wrap:wrap;background:linear-gradient(135deg,rgba(14,27,66,.95),rgba(14,27,66,.75));border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:18px;color:#fff;box-shadow:0 10px 30px rgba(0,0,0,.12)}
.home-title{font-size:1.4rem;font-weight:800;letter-spacing:.2px}
.home-sub{opacity:.8;margin-top:6px;font-size:.95rem;max-width:640px}
.home-hero-right{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-left:auto}
.home-quick-btn{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.18);color:#fff;padding:10px 12px;border-radius:10px;cursor:pointer;font-weight:700;transition:.2s}
.home-quick-btn:hover{background:rgba(255,255,255,.14);transform:translateY(-1px)}
.home-panels{margin-top:16px;display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:14px}
.home-panel{background:#fff;border-radius:12px;border:1px solid #eaedf2;box-shadow:0 8px 20px rgba(0,0,0,.05);overflow:hidden}
.home-panel-head{padding:12px 14px;font-weight:800;color:#0e1b42;background:#f7f9fc;border-bottom:1px solid #eef2f7;display:flex;gap:8px;align-items:center}
.home-panel-body{padding:12px 14px;color:#444;font-size:.95rem;line-height:1.55}
.home-shortcuts{display:flex;gap:10px;flex-wrap:wrap}
.home-shortcut{background:#0e1b42;color:#fff;border:none;border-radius:10px;padding:10px 12px;cursor:pointer;font-weight:800;transition:.2s}
.home-shortcut:hover{transform:translateY(-1px);opacity:.92}
.minimal-input{padding:10px 12px;border:1px solid #e5e7eb;border-radius:10px;outline:none}
.minimal-input:focus{border-color:#fabb00;box-shadow:0 0 0 3px rgba(250,187,0,.18)}


/* Teknik Döküman Kartları */
.doc-stack{
...
.doc-card{
  ...
  border: 1px solid rgba(221,227,255,.9);
  border-radius: 14px;
  overflow: hidden;
