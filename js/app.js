...
// --- WIZARD FUNCTIONS ---
// --- REFUND WIZARD (İade Asistanı) v2 ---
// Not: Eski "WizardSteps" sheet yapısını bozmadan, karar motoru frontend'de çalışır.
// İstersen sonradan kuralları sheet'e de taşıyabiliriz.

const refundWizardState = {
  step: 0,
  answers: {
    paket: null,          // 'aylik' | 'yillik' | 'yillik_kampanyali'
    satinAlma: null,      // 'yeni' | 'oto' | 'winback'
    izleme: null,         // true | false
    genelSorun: null,     // true | false  (genel problem var mı?)
    thh: null,            // true | false
    sure: null,           // '24s' | 'ay15' | 'asimi'
    mukerrer: null,       // true | false
    ayniGunAylikUstuYillik: null, // true | false
    kodIleSatinAlmaTalebi: null,   // true | false
    ucretsizDenemeKod: null,       // true | false
    yilliklocoaFark: null          // true | false
  }
};

function openWizard(){
  document.getElementById('wizard-modal').style.display = 'flex';
  // İlk açılışta sıfırla
  refundWizardReset();
}

function refundWizardReset(){
  refundWizardState.step = 0;
  refundWizardState.answers = {
    paket: null,
    satinAlma: null,
    izleme: null,
    genelSorun: null,
    ...
    sure: null,
    mukerrer: null,
    ayniGunAylikUstuYillik: null,
    kodIleSatinAlmaTalebi: null,
    ucretsizDenemeKod: null,
    yilliklocoaFark: null
  };
  refundWizardRender();
}

function refundWizardBack(){
  refundWizardState.step = Math.max(0, refundWizardState.step - 1);
  refundWizardRender();
}

function rwSet(key, val){
  refundWizardState.answers[key] = val;
  refundWizardState.step += 1;
  refundWizardRender();
}

function rwBadge(label, tone='info'){
  const cls = tone === 'danger' ? 'rw-badge rw-badge-danger'
            : tone === 'warning' ? 'rw-badge rw-badge-warning'
            : tone === 'success' ? 'rw-badge rw-badge-success'
            : 'rw-badge rw-badge-info';
  return `<span class="${cls}">${label}</span>`;
}

function rwBtn(text, onClick, variant='primary'){
  const cls = variant === 'ghost' ? 'rw-btn rw-btn-ghost'
            : variant === 'danger' ? 'rw-btn rw-btn-danger'
            : variant === 'secondary' ? 'rw-btn rw-btn-secondary'
            : 'rw-btn rw-btn-primary';
  return `<button class="${cls}" onclick="${onClick}">${text}</button>`;
}

function rwSection(title, desc){
  return `<div class="rw-section">
    <div class="rw-section-title">${title}</div>
    ${desc ? `<div class="rw-section-desc">${desc}</div>` : ``}
  </div>`;
}

function rwSummaryRow(k, v){
  if (v === null || v === undefined) return '';
  return `<div class="rw-sum-row"><span class="rw-sum-k">${k}</span><span class="rw-sum-v">${v}</span></div>`;
}

function refundWizardDecision(a){
  // Kurallar: "İade Talep Konuları ve Yönlendirme Adımları" dokümanına göre.
  // Çıktı: {status:'red|yellow|green', title, reason, actions:[...], scripts:[...]}
  const out = { status:'yellow', title:'İnceleme', reason:'', actions:[], scripts:[] };

  const within = (a.sure === 'ay15' || a.sure === '24s');

  // 1) İzleme var + genel sorun yok => reddet
  if (a.izleme === true && a.genelSorun === false) {
    out.status = 'red';
    out.title = 'İade RED';
    out.reason = 'İçerik izlenmiş ve genel bir sorun yoksa, iade süreci başlatılmaz.';
    out.actions = ['“cayma” kısayolu paylaş', 'Kullanım olduğu için iade reddi bilgisi ver'];
    out.scripts = [
      'Size yardımcı olmak isterim. Hizmet kullanımı (izleme) olduğu ve genel bir sorun olmadığı için iade işlemi başlatamıyoruz. Dilerseniz cayma/iptal adımlarını paylaşabilirim.'
    ];
    return out;
  }

  // 8) YILLIKLOCA fark iade (müşteri isterse) -> aynı ay & 15 gün şartı
  if (a.yilliklocoaFark === true) {
    if (!within) {
      out.status = 'red';
      out.title = 'Talep RED (Süre Aşımı)';
      out.reason = 'YILLIKLOCA fark iadesi için ücret tahsilatından sonra aynı ay ve 15 gün içinde ulaşmış olmalı.';
      out.actions = ['Talebi nazikçe reddet', 'Gerekirse cayma kısayolu paylaş'];
      out.scripts = [
        'Kontrol ettim; bu talepte işlem yapabilmemiz için ücret tahsilatından sonra aynı ay ve 15 gün içinde başvuru şartı var. Bu süre aşıldığı için işlem sağlayamıyoruz.'
      ];
      return out;
    }
    out.status = 'green';
    out.title = 'Fark İadesi UYGUN';
    out.reason = 'Müşteri YILLIKLOCA fark iadesi talep ediyorsa olumlu yaklaşılır; fark doğrudan iade edilebilir (memnuniyet).';
    out.actions = ['“cayma” kısayolu paylaş', 'Aradaki farkı iade et', 'Memnuniyet kapsamında olduğunu belirt'];
    out.scripts = [
      'Elbette yardımcı olabilirim. Talebiniz doğrultusunda YILLIKLOCA kampanyası ile aradaki farkı memnuniyet kapsamında iade ediyorum. Ayrıca cayma adımlarını da paylaşabilirim.'
    ];
    return out;
  }

  // 4) Mükerrer üyelik (çift satın alım)
  if (a.mukerrer === true) {
    if (!within) {
      out.status = 'red';
      out.title = 'Talep RED (Süre Aşımı)';
      out.reason = 'Mükerrer iade için de ücret tahsilatından sonra aynı ay ve 15 gün içinde ulaşmış olması gerekiyor.';
      out.actions = ['Talebi reddet', 'Gerekirse cayma kısayolu paylaş'];
      out.scripts = [
        'Bu işlem için ücret tahsilatından sonra aynı ay ve 15 gün içinde başvuru şartı bulunuyor. Süre aşıldığı için iade işlemi yapamıyoruz.'
      ];
      return out;
    }
    out.status = 'yellow';
    out.title = 'Mükerrer – Kontrol Gerekli';
    out.reason = 'Aynı cihaz/IP üzerinden kullanım varsa mükerrer hesap için iade yapılabilir; farklı cihaz/IP ise (ve/veya izleme varsa) reddedilir.';
    out.actions = ['2 hesap için cihaz + IP kontrol et', 'Aynı cihaz/IP ise mükerrer olan hesabı iade et', 'Farklıysa talebi reddet'];
    out.scripts = [
      'Mükerrer satın alım kontrolü için iki hesabınızın cihaz/IP kullanımını inceleyip, aynı kullanım varsa mükerrer olan hesap için iade işlemini sağlayacağım.'
    ];
    return out;
  }

  // 5) Aylık üstüne aynı gün yıllık satın alımı (24 saat içinde) -> bir defaya mahsus yıllık iade
  if (a.ayniGunAylikUstuYillik === true) {
    if (a.sure !== '24s') {
      out.status = 'red';
      out.title = 'Talep RED';
      out.reason = 'Aynı gün alınan yıllık paket iadesi sadece 24 saat içinde ulaşıldığında istisnai yapılır.';
      out.actions = ['Talebi reddet', 'Cayma kısayolu paylaş'];
      out.scripts = [
        'Bu tür işlemlerde istisnai iade yalnızca satın alımdan sonraki ilk 24 saat içinde mümkün olabiliyor. Süre aşıldığı için iade işlemi yapamıyoruz.'
      ];
      return out;
    }
    out.status = 'green';
    out.title = 'İstisnai Yıllık İadesi UYGUN';
    out.reason = 'Aylık paketin üzerine aynı gün yıllık alındıysa ve 24 saat içinde ulaştıysa bir defaya mahsus yıllık iade yapılabilir.';
    out.actions = ['“cayma” kısayolu paylaş', '1 defaya mahsus memnuniyet iadesi yap', 'İstisna olduğunu açıkça belirt'];
    out.scripts = [
      'Normalde iade sürecimiz bulunmuyor; ancak memnuniyet kapsamında bir defaya mahsus olmak üzere yıllık paketinizin iadesi konusunda yardımcı olabilirim. Cayma adımlarını da paylaşıyorum.'
    ];
    return out;
  }

  // 6) Winback yıllık paket -> ikinci paketin iadesi (memnuniyet) + süre şartı
  if (a.satinAlma === 'winback') {
    if (!within) {
      out.status = 'red';
      out.title = 'Talep RED (Süre Aşımı)';
      out.reason = 'Winback senaryosunda da ücret tahsilatından sonra aynı ay ve 15 gün şartı var.';
      out.actions = ['Talebi reddet', 'Cayma kısayolu paylaş'];
      out.scripts = [
        'Winback işlemlerinde ücret tahsilatından sonra aynı ay ve 15 gün içinde başvuru şartı bulunuyor. Süre aşıldığı için işlem sağlayamıyoruz.'
      ];
      return out;
    }
    out.status = 'green';
    out.title = 'Winback – 2. Paket İadesi UYGUN';
    out.reason = 'Winback yıllık paket alındıysa memnuniyet amacıyla sadece ikinci paketin iadesi sağlanabilir.';
    out.actions = ['“cayma” kısayolu paylaş', 'Sadece ikinci paketi iade et', 'Memnuniyet kapsamını belirt'];
    out.scripts = [
      'Memnuniyet kapsamında, Winback ile alınan ikinci paket için iade işlemini sağlayabilirim. Cayma adımlarını da paylaşıyorum.'
    ];
    return out;
  }

  // 7) Yenilenen paket yerine kampanya kodu ile satın alma talebi
  if (a.kodIleSatinAlmaTalebi === true) {
    if (!within) {
      out.status = 'red';
      out.title = 'Talep RED (Faturalama/Süre)';
      out.reason = 'Kampanya kodu talebinde ücret tahsilatından sonra aynı ay ve 15 gün içinde ulaşma şartı var; farklı aya sarkarsa reddedilir.';
      out.actions = ['Talebi reddet', 'Gerekirse YILLIKLOCA alternatifi sun'];
      out.scripts = [
        'Bu talepte işlem sağlayabilmemiz için ücret tahsilatından sonra aynı ay ve 15 gün içinde başvuru şartı var. Bu şart sağlanmadığı için işlem yapamıyoruz. Gerekirse YILLIKLOCA alternatifi üzerinden destek olabilirim.'
      ];
      return out;
    }
    // ücretsiz deneme / çark kodu ise reddet, YILLIKLOCA öner
    if (a.ucretsizDenemeKod === true) {
      out.status = 'red';
      out.title = 'Talep RED (Kod Tipi)';
      out.reason = 'Ücretsiz deneme süresi olan veya çark kampanyası gibi kodlarla bu işlem reddedilir; gerekirse YILLIKLOCA ile destek olunur.';
      out.actions = ['Talebi reddet', 'YILLIKLOCA ile destek ol'];
      out.scripts = [
        'Paylaştığınız kod ücretsiz deneme/özel kampanya türünde olduğu için bu işlem kapsamında kullanamıyoruz. Dilerseniz YILLIKLOCA kampanyası ile destek olabilirim.'
      ];
      return out;
    }
    out.status = 'yellow';
    out.title = 'Kod ile Satın Alma – Uygulanabilir Akış';
    out.reason = 'Kullanıcı kod ile satın alma yaptıktan sonra ilk paket iade edilip, son paketin tarihi düzenlenebilir (uygun şartlarda).';
    out.actions = ['Müşterinin kod ile satın alımını doğrula', 'İlk paketi iade et', 'Son paketin başlangıç tarihini düzenle'];
    out.scripts = [
      'Kod ile satın alımınızı doğruladıktan sonra önceki paketin iadesini yapıp, yeni paketin başlangıç tarihini düzenleyerek yardımcı olabilirim.'
    ];
    return out;
  }

  // 2) Aylık + izleme yok + THH -> önce 6AY50 fark iadesi, kabul etmezse tam iade (çağrıdan)
  if (a.paket === 'aylik' && a.izleme === false) {
    // süre filtresi dokümanda “THH deyince” gibi; genel kural: aynı ay/15 gün şartı kampanya/fark senaryolarında var.
    out.status = 'yellow';
    out.title = 'Aylık – İzleme Yok';
    out.reason = 'İzleme geçmişi yoksa; THH ile gelirse önce 6AY50 kapsamında indirim iadesi teklif edilir; kabul etmezse tam iade yapılabilir.';
    out.actions = ['“oto yenileme” + “cayma” kısayolu paylaş (yenileme ise)', '6AY50 (139,5) fark iadesini teklif et', 'Kabul etmezse tam iade'];
    out.scripts = [
      'İzleme olmadığı için yardımcı olabilirim. Öncelikle 6AY50 kampanyası kapsamında indirim iadesi (139,5 TL) teklif edebilirim; kabul etmezseniz tam iade için işlem başlatabilirim.'
    ];
    return out;
  }

  // 3) Yıllık + izleme yok
  if ((a.paket === 'yillik' || a.paket === 'yillik_kampanyali') && a.izleme === false) {
    out.status = 'yellow';
    out.title = 'Yıllık – İzleme Yok';
    out.reason = 'Yenileme ise otoyenileme+cayma paylaşılır. THH ile gelirse önce YILLIKLOCA indirim teklif edilir. Kampanyalı yıllıkta önce aylığa geçiş teklifi yapılabilir.';
    out.actions = [
      'Yenileme ise “otoyenileme” + “cayma” kısayolu paylaş',
      'NotificationHistory varsa yenileme e-postası görselini gönder',
      (a.paket === 'yillik' ? 'YILLIKLOCA indirimi teklif et' : 'Önce aylık pakete geçiş teklif et (kalan tutarı iade + aylık atama)'),
      'Kabul olmazsa tam iade'
    ];
    out.scripts = [
      (a.paket === 'yillik'
        ? 'İzleme olmadığı için yardımcı olabilirim. Öncelikle YILLIKLOCA kampanyası ile indirim/fark desteği sunayım; dilerseniz aylık pakete geçiş veya tam iade seçeneklerini de değerlendirebiliriz.'
        : 'İzleme olmadığı için yardımcı olabilirim. Öncelikle yıllık paketinizi aylık pakete çevirip, yıllık ücret ile aylık ücret arasındaki kalan tutarı iade ederek ilerleyebiliriz. Kabul etmezseniz tam iade için işlem başlatabiliriz.')
    ];
    return out;
  }

  // Default / eksik cevaplar
  out.status = 'yellow';
  out.title = 'Eksik Bilgi';
  out.reason = 'Seçimler tamamlanmadan net karar verilemez.';
  out.actions = ['Soruları tamamla'];
  out.scripts = ['Detayları birlikte netleştirip en doğru işlemi sağlayalım.'];
  return out;
}

function refundWizardRender(){
  const b = document.getElementById('wizard-body');
  if(!b) return;

  const a = refundWizardState.answers;
  const step = refundWizardState.step;

  // Üst bar (başlık + özet)
  let h = `
    <div class="rw-header">
      <div class="rw-title">
        <div style="font-size:1.25rem;font-weight:900;color:var(--primary);">🤖 İade Asistanı</div>
        <div class="rw-sub">Soru → Karar → Aksiyon → Kopyala</div>
      </div>
      <div class="rw-actions">
        ${rwBtn('<i class="fas fa-redo"></i> Sıfırla', 'refundWizardReset()', 'ghost')}
      </div>
    </div>

    <div class="rw-card">
      <div class="rw-card-left">
        ${rwSection('Durum Özeti', 'Seçtiklerin burada birikir')}
        <div class="rw-summary">
          ${rwSummaryRow('Paket', a.paket ? a.paket.replace('yillik_kampanyali','Yıllık (Kampanyalı)').replace('yillik','Yıllık').replace('aylik','Aylık') : '—')}
          ${rwSummaryRow('Satın Alma', a.satinAlma ? (a.satinAlma==='oto'?'Oto Yenileme':a.satinAlma==='winback'?'Winback':'Yeni') : '—')}
          ${rwSummaryRow('İzleme', a.izleme===null?'—':(a.izleme?'Var':'Yok'))}
          ${rwSummaryRow('Genel Sorun', a.genelSorun===null?'—':(a.genelSorun?'Var':'Yok'))}
          ${rwSummaryRow('THH', a.thh===null?'—':(a.thh?'Evet':'Hayır'))}
          ${rwSummaryRow('Süre', a.sure ? (a.sure==='24s'?'24 saat':a.sure==='ay15'?'Aynı ay/15 gün':'Süre aşımı') : '—')}
        </div>
        <div class="rw-mini-note">
          Not: “İzleme var + genel sorun yok” ise iade reddi. “Aynı ay/15 gün” kuralı kampanya/istisna senaryolarında kritik.
        </div>
      </div>

      <div class="rw-card-right">
  `;
  ...
  const steps = [
    {
      key: 'paket',
      title: 'Paket türü nedir?',
      desc: 'Aylık / Yıllık / Kampanyalı Yıllık',
      options: [
        {t:'Aylık', v:'aylik'},
        {t:'Yıllık', v:'yillik'},
        {t:'Yıllık (Kampanyalı)', v:'yillik_kampanyali'}
      ]
    },
    {
      key: 'satinAlma',
      title: 'Satın alma şekli?',
      desc: 'Yenileme/Winback ayrımı sonraki kuralları etkiler.',
      options: [
        {t:'Yeni satın alma', v:'yeni'},
        {t:'Otomatik yenileme', v:'oto'},
        {t:'Winback yıllık paket', v:'winback'}
      ]
    },
    {
      key: 'izleme',
      title: 'İzleme geçmişi var mı?',
      desc: 'İçerik izlendiyse (genel sorun yoksa) iade reddedilir.',
      options: [
        {t:'Var', v:true},
        {t:'Yok', v:false}
      ]
    ...
    {
      key: 'genelSorun',
      title: 'Genel bir sorun var mı?',
      desc: 'Yayın açılmıyor, sürekli hata vb. gibi genel problem.',
      options: [
        {t:'Var', v:true},
        {t:'Yok', v:false}
      ]
    ...
    {
      key: 'sure',
      title: 'Müşteri ne zaman ulaştı?',
      desc: 'Bazı işlemler için “aynı ay ve 15 gün” şartı var.',
      options: [
        {t:'24 saat içinde', v:'24s'},
        {t:'Aynı ay / 15 gün içinde', v:'ay15'},
        {t:'Süre aşımı / farklı aya sarktı', v:'asimi'}
      ]
    },
    {
      key: 'thh',
      title: 'THH (Tüketici Hakem Heyeti) diyor mu?',
      desc: 'Bazı senaryolarda önce teklif/alternatif sunulur.',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false}
      ]
    ...
    {
      key: 'mukerrer',
      title: 'Mükerrer (çift satın alım) durumu var mı?',
      desc: '2 hesap / 2 satın alım vb.',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false}
      ]
    ...
    {
      key: 'ayniGunAylikUstuYillik',
      title: 'Aylık üstüne aynı gün yıllık paket alınmış mı?',
      desc: '24 saat içinde ulaştıysa istisnai yıllık iade olabilir.',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false}
      ]
    ...
    {
      key: 'kodIleSatinAlmaTalebi',
      title: 'Yenilenen paket yerine kampanya kodu ile satın alma talebi var mı?',
      desc: 'Kod ile satın alım yapıldıktan sonra ilk paket iade + tarih düzenleme akışı olabilir.',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false}
      ]
    ...
    {
      key: 'ucretsizDenemeKod',
      title: 'Kod ücretsiz deneme / çark kampanyası gibi mi?',
      desc: 'Bu tip kodlarda talep reddedilir; gerekirse YILLIKLOCA önerilir.',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false},
        {t:'Bilmiyorum', v:false} // güvenli varsayım
      ],
      showIf: () => a.kodIleSatinAlmaTalebi === true
    },
    {
      key: 'yilliklocoaFark',
      title: 'Müşteri “YILLIKLOCA fark iadesi” özellikle istiyor mu?',
      desc: 'Müşteri talebiyle gelirse fark iadesi yapılabilir (süre şartıyla).',
      options: [
        {t:'Evet', v:true},
        {t:'Hayır', v:false}
      ]
  ...
  // Adımları koşullu göster
  let visibleSteps = steps.filter(s => !s.showIf || s.showIf());

  // Karar ekranı: tüm zorunlular dolduysa
  const mustKeys = ['paket','satinAlma','izleme','genelSorun','sure','thh','mukerrer','ayniGunAylikUstuYillik','kodIleSatinAlmaTalebi','yilliklocoaFark'];
  const done = mustKeys.every(k => a[k] !== null);

  if (!done) {
    // sıradaki adımı bul: null olan ilk
    const nextStep = visibleSteps.find(s => a[s.key] === null) || visibleSteps[0];
    const progress = Math.round((mustKeys.filter(k => a[k] !== null).length / mustKeys.length) * 100);

    h += `
      ${rwBadge(`İlerleme: %${progress}`, progress<50?'warning':(progress<90?'info':'success'))}
      <div class="rw-q-title">${nextStep.title}</div>
      <div class="rw-q-desc">${nextStep.desc || ''}</div>
      <div class="rw-grid">
        ${nextStep.options.map(o => `
          <button class="rw-opt" onclick="rwSet('${nextStep.key}', ${typeof o.v === 'string' ? `'${o.v}'` : o.v})">
            <div class="rw-opt-title">${o.t}</div>
          </button>
        `).join('')}
      </div>
      <div class="rw-footer">
        ${refundWizardState.step > 0 ? rwBtn('<i class="fas fa-arrow-left"></i> Geri', 'refundWizardBack()', 'ghost') : ''}
        <div style="margin-left:auto;opacity:.7;font-size:.85rem;">Adım: ${mustKeys.filter(k=>a[k]!==null).length}/${mustKeys.length}</div>
      </div>
    `;
  } else {
    const d = refundWizardDecision(a);
    const boxCls = d.status === 'red' ? 'res-red' : (d.status === 'green' ? 'res-green' : 'res-yellow');
    const icon = d.status === 'red' ? '🛑' : (d.status === 'green' ? '✅' : '⚠️');

    h += `
      ${rwBadge('Karar Hazır', d.status==='red'?'danger':(d.status==='green'?'success':'warning'))}
      <div class="rw-q-title">${icon} ${d.title}</div>
      <div class="rw-q-desc">${d.reason}</div>

      <div class="result-box ${boxCls}" style="text-align:left;">
        <div style="font-weight:900;margin-bottom:8px;">Yapılacaklar</div>
        <ul class="rw-ul">
          ${(d.actions||[]).map(x => `<li>${x}</li>`).join('')}
        </ul>
      </div>

      <div class="rw-copy-area">
        <div style="font-weight:900;margin:10px 0 6px;">Hazır Metin</div>
        ${(d.scripts||[]).map((t,i) => `
          <div class="rw-script">
            <div class="rw-script-text" id="rw-script-${i}">${(t||'').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
            <button class="rw-copy-btn" onclick="copyText('${escapeForJsString(t||'')}')"><i class="fas fa-copy"></i> Kopyala</button>
          </div>
        `).join('')}
      </div>

      <div class="rw-footer">
        ${rwBtn('<i class="fas fa-redo"></i> Başa Dön', 'refundWizardReset()', 'secondary')}
      </div>
    `;
  }
